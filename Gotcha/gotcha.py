	...
	def open_file(self):
		...
		if ext in ['txt', 'py', 'html', 'css', 'js']:
			...
			if 'evince' in self.main_window.all_program:
				...
				self.problem_message()
		elif ext in ['png', 'jpeg', 'jpg', 'gif', 'ico']:
...
class MainWindow():
	...
	def __init__(self):
		...
		self.canvas.configure(yscrollcommand=self.scrollbar_vert.set, xscrollcommand = self.scrollbar_hor.set, width=1000, heigh=750)
		...
		for item in dir_list:
			...
			if os.path.isdir(str(path) + item):
				...
				if (not self.hidden_dir.get() and not item.startswith('.')) or self.hidden_dir.get():
					...
					#фото, картинки
					if ext in ['jpeg', 'jpg', 'png', 'gif', 'ico']:
						...
						#другие файлы
						if ext in ['mp4', 'mov', 'avi', 'wmv', 'avchd', 'flv', 'f4v', 'swf', 'mkv', 'webm']:
							photo = tkinter.PhotoImage(file ="img/video.png")
						...
						elif ext in ['mp3', 'aac', 'aiff', 'dsd', 'flac', 'mqa', 'ogg', 'wav', 'wma']:
							photo = tkinter.PhotoImage(file ="img/sound.png")
							...
							file_name.bind("<Button-3>", self.file_context_menu.popup_menu)
						elif ext in ['txt', 'htm', 'html', 'otd', 'rtf']:
							photo = tkinter.PhotoImage(file ="img/txt.png")
						...
						elif os.access(str(path) + item, os.R_OK):
